return {
	...
	config = function()
		...
		local function get_dlv_cmd()
			...
			vim.notify(
				"dlv no está instalado. Instálalo con Nix (pkgs.delve) o con :MasonInstall delve.",
		...
		if ts_runtime_executable then
			...
			ts_launch_aws.runtimeExecutable = ts_runtime_executable
		...
		dap.configurations.typescript = { clone(ts_launch), clone(ts_launch_env), clone(ts_launch_aws), clone(node_attach) }
		dap.configurations.typescriptreact = { clone(ts_launch), clone(ts_launch_env), clone(ts_launch_aws), clone(node_attach) }
