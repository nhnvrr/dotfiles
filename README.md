...
4. **Open Ghostty** (not Terminal.app) — it picks up `ghostty/config`, the Solarized Dark theme, the fonts, and zsh as the login shell.
...
| Theme | Solarized Dark, via Ghostty's bundled `iTerm2 Solarized Dark` (the canonical Schoonover palette — base03 `#002b36` background, base02 `#073642` panels, orange `#cb4b16`). The background is the theme's own, not overridden: Solarized's contrast ratios are built against base03. The one override left is slot `208`, because Solarized defines only slots 0-15 and tmux paints the bell status bar with 208 — it has to sit **below** the `theme` line to win. nvim runs `solarized.nvim` (transparent, so the terminal background shows through). Starship, eza, `bat`, delta, fzf, pgcli and tmux all resolve colours through the terminal's ANSI slots, so changing the Ghostty theme moves everything at once. GUI editors keep their own user-level theme | [`ghostty/config`](./ghostty/config) |
