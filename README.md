...
| Multiplexer | tmux — no plugins, solid green bar, and nothing is bold. Emphasis is carried by `reverse`, which is also what keeps it legible in both states: the session badge and the active window invert against the bar, so they resolve themselves against the green and against the yellow a bell turns it, with no second conditional. The bell is the whole bar; `#F`'s `!` says which window rang, which is why `window-status-bell-style` is forced back to `default` — tmux's own default there is `reverse`, and on the yellow bar that reads exactly like the active window. The prefix turns the session badge cyan, a zoomed pane flags `Z`, and the active pane border is green | [`tmux/tmux.conf`](./tmux/tmux.conf) |
