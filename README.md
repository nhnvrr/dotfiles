...
| Multiplexer | tmux — no plugins, and the status bar runs on `bg=default` so it passes Ghostty's translucency instead of being the one opaque strip in the window. Colour carries the state and nothing is bold: the active window gets a green `▌`, the inactive ones sit in brightblack, the active pane border is green, and a bell turns the whole bar yellow — louder from a dark bar than it ever was from a green one. The prefix paints the session name and a zoomed pane flags `Z`, both in status-left | [`tmux/tmux.conf`](./tmux/tmux.conf) |
