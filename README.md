...
4. **Set up the Terminal.app profile** by hand — a dark profile, font `MonaspiceAr Nerd Font Mono` at 18, and *Use Option as Meta key* under Keyboard. It's the one layer the script can't do for you (see [What is NOT managed](#what-is-not-managed)); the login shell is already fish by then.
...
| Terminal | Terminal.app (macOS built-in) | — (profile is user-level) |
| Multiplexer | tmux | [`tmux/tmux.conf`](./tmux/tmux.conf) |
| Editor | VS Code (`e` = `code --new-window`) — it's what handles the EC2 work: Remote-SSH, AWS Toolkit and SSM sessions. It's also what Hammerspoon's `cmd+alt+1` layout pairs with Brave. Its config is user-level, not versioned here | — |
...
- **Terminal.app** — the profile carries the font, the size, the sixteen ANSI colours and *Use Option as Meta key*, and it lives in `com.apple.Terminal`'s plist as `NSKeyedArchiver` blobs. An exported `.terminal` file is a plist full of base64: nothing you can diff or edit, so it stays out. Set it once from Settings → Profiles; everything else in this repo reads colours off whatever profile is active.
